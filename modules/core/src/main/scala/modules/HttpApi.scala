...
sealed abstract class HttpApi[F[_]: Async] private (services: Services[F]):
  ...
  private val openRoutes: HttpRoutes[F] = movieDetailRoutes <+> movieRatingRoutes <+> kevinBaconRoutes

  private val routes: HttpRoutes[F] = Router(version.v1 -> openRoutes)

  private val middleware: HttpRoutes[F] => HttpRoutes[F] = { (http: HttpRoutes[F]) => AutoSlash(http) } andThen {
    (http: HttpRoutes[F]) => CORS(http)
